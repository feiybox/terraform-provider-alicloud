...
func TestAccAlicloudNASSmbAclAttachment_basic0(t *testing.T) {
	...
	resourceId := "alicloud_nas_smb_acl_attachment.default"
	checkoutSupportedRegions(t, true, connectivity.NASSupportRegions)
	ra := resourceAttrInit(resourceId, AlicloudNASSmbAclMap0)
	...
	name := fmt.Sprintf("tf-testacc%ssmbacl%d", defaultRegionToTest, rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudNASSmbAclBasicDependence0)
...
func TestUnitAlicloudNASSmbAclAttachment(t *testing.T) {
	...
	dCreate, _ := schema.InternalMap(p["alicloud_nas_smb_acl_attachment"].Schema).Data(nil, nil)
	...
	responseMock := map[string]func(errorCode string) (map[string]interface{}, error){
		...
		"NotFoundError": func(errorCode string) (map[string]interface{}, error) {
			return nil, GetNotFoundErrorFromString(GetNotFoundMessage("alicloud_nas_smb_acl_attachment", "file_system_id"))
		},
	...
	t.Run("CreateClientAbnormal", func(t *testing.T) {
		...
		err := resourceAlicloudNasSmbAclAttachmentCreate(d, rawClient)
		patches.Reset()
		assert.NotNil(t, err)
	})
	...
	t.Run("CreateNormal", func(t *testing.T) {
		...
		err := resourceAlicloudNasSmbAclAttachmentCreate(dCreate, rawClient)
		patches.Reset()
		assert.Nil(t, err)
	})

	// Set ID for Update and Delete Method
	d.SetId(fmt.Sprint("file_system_id", ":", "keytab", ":", "keytab_md5"))
	// Update
	t.Run("UpdateClientAbnormal", func(t *testing.T) {
		...
		err := resourceAlicloudNasSmbAclAttachmentUpdate(d, rawClient)
		patches.Reset()
		assert.NotNil(t, err)
	})
	...
	t.Run("UpdateModifySmbAclAbnormal", func(t *testing.T) {
		...
		resourceData1, _ := schema.InternalMap(p["alicloud_nas_smb_acl_attachment"].Schema).Data(nil, diff)
	...
	t.Run("DeleteClientAbnormal", func(t *testing.T) {
		...
		err := resourceAlicloudNasSmbAclAttachmentDelete(d, rawClient)
		patches.Reset()
		assert.NotNil(t, err)
	})
	...
	//Read
	t.Run("ReadDescribeNasSmbAclNotFound", func(t *testing.T) {
		...
		err := resourceAlicloudNasSmbAclAttachmentRead(d, rawClient)
		patcheDorequest.Reset()
		assert.Nil(t, err)
	})

	t.Run("ReadDescribeNasSmbAclAbnormal", func(t *testing.T) {
		patcheDorequest := gomonkey.ApplyMethod(reflect.TypeOf(&client.Client{}), "DoRequest", func(_ *client.Client, _ *string, _ *string, _ *string, _ *string, _ *string, _ map[string]interface{}, _ map[string]interface{}, _ *util.RuntimeOptions) (map[string]interface{}, error) {
			retryFlag := false
			noRetryFlag := true
			if retryFlag {
